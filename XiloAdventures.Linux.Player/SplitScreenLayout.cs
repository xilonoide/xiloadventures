...
/// Con soporte para imagen ASCII en la parte superior
/// </summary>
public class SplitScreenLayout
{
    ...
    // Image section properties - currently disabled but preserved for future use.
    // Altura de la sección de imagen (1/3 de la pantalla, incluyendo bordes)
    private static int ImageSectionHeight => Height / 3;

    // Altura del área de paneles (2/3 de la pantalla)
    private static int PanelSectionHeight => Height - ImageSectionHeight;
    ...
    private int GetLeftContentHeight(bool hasImage) => (hasImage ? PanelSectionHeight : Height) - 7;
    ...
    public void Render(GameState state, Room? room, WorldModel world)
    {
        ...
        // [RESERVED FOR FUTURE USE - DO NOT DELETE]
        // ASCII image display is currently disabled. To re-enable:
        // var hasImage = !string.IsNullOrEmpty(room?.AsciiImage);
        var hasImage = false; // Disabled - no image section
        var contentHeight = GetLeftContentHeight(hasImage);

        // === SECCIÓN DE IMAGEN (si existe) ===
        if (hasImage)
        {
            var imageLines = GetImageLines(room!.AsciiImage!, TotalWidth - 2, ImageSectionHeight - 2);

            // Borde superior de la imagen
            sb.Append(ConsoleRenderer.TopLeft);
            sb.Append(new string(ConsoleRenderer.Horizontal, TotalWidth - 2));
            ...
            foreach (var line in imageLines)
            {
                ...
                sb.Append(PadRight(line, TotalWidth - 2));
                sb.Append(ConsoleRenderer.Vertical);
                sb.AppendLine();
            }

            // Separador entre imagen y paneles (conecta con los paneles)
    ...
    public void UpdateRightPanel(GameState state, WorldModel world, bool hasImage = false)
    {
        ...
        int rowOffset = hasImage ? ImageSectionHeight : 0;
    ...
    /// <summary>
    /// Prepara las líneas de la imagen ASCII para mostrar.
    /// [RESERVED FOR FUTURE USE - DO NOT DELETE]
    /// This method is currently unused but preserved for future ASCII image display.
    /// </summary>
    private List<string> GetImageLines(string asciiImage, int maxWidth, int maxHeight)
    {
        var result = new List<string>();

        // Dividir la imagen en líneas
        var allLines = asciiImage.Split('\n', StringSplitOptions.None);

        // Calcular cuántas líneas de la imagen podemos mostrar
        // y desde dónde empezar si la imagen es más alta
        int startLine = 0;
        int linesToTake = Math.Min(allLines.Length, maxHeight);

        // Si la imagen es más alta que el espacio disponible, mostrar desde el centro
        if (allLines.Length > maxHeight)
        {
            startLine = (allLines.Length - maxHeight) / 2;
        }

        // Procesar las líneas
        for (int i = 0; i < linesToTake && startLine + i < allLines.Length; i++)
        {
            var line = allLines[startLine + i].TrimEnd('\r');

            // Centrar la línea si es más corta que el ancho disponible
            if (line.Length < maxWidth)
            {
                int leftPadding = (maxWidth - line.Length) / 2;
                line = new string(' ', leftPadding) + line;
            }
            // Truncar si es más larga
            else if (line.Length > maxWidth)
            {
                int startPos = (line.Length - maxWidth) / 2;
                line = line.Substring(startPos, maxWidth);
            }

            // Aplicar color verde para efecto terminal/matrix
            result.Add($"{Colors.Green}{line}{Colors.Reset}");
        }

        // Rellenar con líneas vacías si la imagen es más pequeña
        while (result.Count < maxHeight)
        {
            result.Add("");
        }
